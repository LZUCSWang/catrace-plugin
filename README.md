...
### ✅ 自动签到（auto-checkin）
每天为每个浏览器随机分配一个签到时刻，到点自动运行签到脚本。

- 窗口（默认 09:00–21:00）被均分为 N 段，每个浏览器在专属时段内随机取点，签到时间天然错开。
- 面板随时查看每个浏览器的计划时刻与签到结果；支持立即测试、单浏览器重试、一键全部补跑、暂停。
- 错过时刻（睡眠/关机）后在窗口内唤醒即补跑；全部结束后弹当日汇总卡。
- 需要本机装 Node.js；签到能力来自插件内置的 PowerShell 脚本（可配置自定义脚本路径）。
...
- 卡片上能看是谁发的、按 App 区分图标；不想要的 App 可以直接在卡片上点 **屏蔽此应用**，只想拦某个群 / 某条标题就点 **屏蔽这个标题**。
- 不想看的 App 或群，可在卡片上屏蔽，也可在设置里增删。
- 验证码单独弹短卡；QQ / 微信等聊天合成对话小窗：弹出时只显示最新一条，新消息往上顶；往上滚再加载本机历史。分段间隔显示时间戳。
